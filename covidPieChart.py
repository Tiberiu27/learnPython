    ...
    for i in w['covid_romania']:
        for v in i.values():
            if v == userDate:
                ...
                print(str(cases) + ' cases confirmed from ' + str(tests) + ' tests, that means ' + str(
                    round(cases / (tests / 100), 1)) + '%')
...
def time_walking():
    ...
    for i in range(14):
        if (intDay == 0 and intMonth == 10) or (intDay == 0 and intMonth == 12) or (intDay == 0 and intMonth == 5) or (
                intDay == 0 and intMonth == 7):
            intDay = 30
            intMonth -= 1
            ...
            intDay = 31
            intMonth -= 1
        days.append(year + '-' + str(intMonth).rjust(2, '0') + '-' + str(intDay).rjust(2,
                                                                                       '0'))  # the rjust(2, '0') puts a 0 on the left if the value is single digit.
        intDay -= 1
    return days
...
totalCounties = [county['county_name'].lower() for county in
                 countyDict]  # Replacing the for loop with a list comprehension

# if user enters a valid county, create new sheet and fill it with data from 14 days.
if userCounty.lower() in totalCounties:
    timeCases = timeline_selector()
